...
type ClassRow = { id: string; name: string; level: string | null; created_at: string; updated_at: string }
type AssignmentRow = {
  id: string
...
function StudentDashboardInner() {
  ...
  async function loadClasses() {
    ...
    const { data, error } = await supabase.from("classes").select("id,name,level,created_at,updated_at").order("created_at", { ascending: false })
  ...
  async function loadAssignments() {
    ...
    try {
      ...
      const direct = await supabase
        ...
        .select("id,assessment_id,class_id,user_id,opens_at,due_at,created_at,assessments(title)")
        .eq("user_id", user.id)
  ...
  useEffect(() => { reload() }, [])
  useEffect(() => { if (!loading) loadAssignments() }, [classes]) // refresh assignments after classes load

  async function redeem() {
    const code = invite.trim().toUpperCase()
    if (!code) return
    const { data, error } = await supabase.rpc("redeem_class_invite", { p_code: code })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Join with code */}
      <Card>
        <CardHeader><CardTitle>Join a class</CardTitle></CardHeader>
        <CardContent className="flex flex-col gap-3 sm:flex-row">
          ...
          />
          <Button onClick={redeem} disabled={!invite.trim()}>Apply code</Button>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-sm text-muted-foreground">You are not in any class yet. Use an invite code above.</div>
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              {classes.map(c => (
                <div key={c.id} className="rounded-md border p-3">
                  ...
                  <div className="text-xs text-muted-foreground">Joined / Updated: {new Date(c.updated_at).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {assignments.map(a => {
                ...
                const due = a.due_at ? new Date(a.due_at).toLocaleString() : "—"
                return (
                  <div key={a.id} className="rounded-md border p-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {a.class_id ? <>Class: {cls?.name || a.class_id}</> : <>Personal assignment</>}
                    </div>
                  </div>
                )
              })}
