...
export default function AdminUsers() {
	...
	useEffect(() => {
		if (allowed) load();
	}, [allowed]); // initial load after access check
	useEffect(() => {
		if (!allowed) return;
		const t = setTimeout(load, 250);
		return () => clearTimeout(t);
	}, [q, roleFilter, allowed]); // debounced
	...
	async function changeRole(userId: string, newRole: Role) {
		...
		if (userId === profile?.id && newRole !== "superadmin") {
			...
			if (last) {
				alert(
					"You are the last superadmin. Add another superadmin before demoting yourself.",
				);
				return;
			}
			if (
				!confirm(
					"You are about to remove your superadmin role from yourself. Continue?",
				)
			)
				return;
		}
								...
								<tr key={r.id} className="border-t">
									...
									<td className="p-3">
										<div className="flex gap-2">
											<Button
												size="sm"
												variant={r.role === "student" ? "default" : "outline"}
												disabled={saving === r.id || r.role === "student"}
												onClick={() => changeRole(r.id, "student")}
											>
												Make student
											</Button>
											<Button
												size="sm"
												variant={r.role === "teacher" ? "default" : "outline"}
												disabled={saving === r.id || r.role === "teacher"}
												onClick={() => changeRole(r.id, "teacher")}
											>
												Make teacher
											</Button>
											<Button
												size="sm"
												variant={
													r.role === "superadmin" ? "default" : "outline"
												}
												disabled={saving === r.id || r.role === "superadmin"}
												onClick={() => changeRole(r.id, "superadmin")}
											>
												Make superadmin
											</Button>
										</div>
