...
export default function RedeemInvite() {
	...
	useEffect(() => {
		if (!profile) {
			...
			navigate(
				`/auth/student?returnTo=${encodeURIComponent(window.location.pathname)}`,
			);
		}
	}, [profile, navigate]);
					...
					{redeemed ? (
						<div className="text-center">
							<p className="text-muted-foreground mb-4">
								You've successfully joined the class! Redirecting to your
								dashboard...
							</p>
						</div>
					) : error ? (
						...
						<div className="text-center">
							<p className="text-muted-foreground mb-4">
								You're about to join a class using invite code:{" "}
