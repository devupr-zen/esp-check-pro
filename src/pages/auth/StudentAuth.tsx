	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (mode === "signup") {
				...
				if (!studentInvite) {
					const { data: validationData, error: inviteError } =
				...
				if (studentInvite && authData.user) {
					const { error: useStudentInviteError } = await supabase.rpc(
						"use_student_invite",
						{
					...
					if (useStudentInviteError)
						console.error("Error using student invite:", useStudentInviteError);
				} else {
