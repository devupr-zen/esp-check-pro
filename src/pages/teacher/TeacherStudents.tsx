...
export default function TeacherStudents() {
		...
		try {
			...
			if ((classesData ?? []).length > 0) {
				...
				const { data: inv, error: invErr } = await supabase
					.from("class_invites")
					.select(
						"id,code,class_id,max_uses,used_count,expires_at,revoked,created_at,recipient_name,recipient_email",
					)
					.in("class_id", classIds)
					.eq("created_by", myId)
					.order("created_at", { ascending: false });
	...
	const handleInviteStudent = async () => {
		...
		try {
			...
			const { data: inviteRow, error: inviteErr } = await supabase.rpc(
				"create_class_invite",
				{
	...
	const handleResendInvite = async (inviteCode: string) => {
		try {
			...
			const { data: inv, error } = await supabase
				.from("class_invites")
				.select(
					"code, class_id, expires_at, revoked, recipient_name, recipient_email",
				)
				.eq("code", inviteCode)
				.single();
			...
			if (inv?.recipient_email) {
				...
				toast({
					title: "Success",
					description: `Invitation resent to ${inv.recipient_email}`,
				});
			} else {
				await navigator.clipboard.writeText(link);
				toast({
					title: "Link copied",
					description: "Invite has no stored email—share the link manually.",
				});
			}
		} catch (error: any) {
	...
	const filteredData = allStudentsAndInvites.filter(
		(item) =>
										...
										<div className="flex items-center justify-end space-x-2">
											{item.type === "invite" && (
												<Button
													...
													variant="outline"
													onClick={() =>
														handleResendInvite("code" in item ? item.code : "")
													}
													disabled={submitting}
											...
											{item.type === "student" && item.status === "active" && (
												<Button
													...
													variant="outline"
													onClick={() =>
														handleRemoveStudent(item.id, item.class_id || "")
													}
													disabled={submitting}
