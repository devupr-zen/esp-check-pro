...
export default function TeacherAssignmentDetail() {
	...
	const {
		...
		queryFn: async () => {
			const { data, error } = await supabase
				...
				.select(
					"id, assignment_id, status, score, feedback, updated_at, created_by, submission_file_path",
				)
	...
	const createdByIds = useMemo(
		() =>
			Array.from(
				new Set((submissions ?? []).map((s) => s.created_by).filter(Boolean)),
			) as string[],
		[submissions],
	);
	...
	const openAttachment = async (path?: string | null) => {
		...
		try {
			const resp = await fetch(
				`/api/storage/sign-download?path=${encodeURIComponent(path)}`,
			);
			...
			<div className="p-6">
				<GlassCard className="p-6">
					...
					<div className="text-sm opacity-80 mt-1">
						{(errA as any)?.message ?? "Unknown error"}
					</div>
					<Button
						variant="secondary"
						className="mt-4"
		...
		<div className="p-6 space-y-4">
			...
			{errorS ? (
				<GlassCard className="p-6">
					...
					<div className="text-sm opacity-80 mt-1">
						{(errS as any)?.message ?? "Unknown error"}
					</div>
				</GlassCard>
			) : (
				<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
					{(submissions ?? []).map((s) => {
						const prof = s.created_by
							? profileById.get(s.created_by)
							: undefined;
						const label = prof?.full_name
							...
							: prof?.email
								? prof.email
								: s.created_by
									? `Student ${s.created_by.slice(0, 8)}`
									: "Student";
