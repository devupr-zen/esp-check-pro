...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !OPENAI_API_KEY) {
  ...
  console.warn('[generate-esp-profile] Missing required env vars. Set SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, OPENAI_API_KEY.');
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({
      ok: true,
      profile: { strengths, weaknesses, improvement_areas, top_goals, scores, level, summary, track: track ?? null },
    });
  } catch (err: any) {
    // eslint-disable-next-line no-console
...
function toStrArray(x: any): string[] {
  if (Array.isArray(x)) return x.filter((s) => typeof s === 'string').slice(0, 5);
