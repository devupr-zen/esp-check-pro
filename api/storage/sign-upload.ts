...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (!userId || !assignmentId || !filename) return res.status(400).json({ error: 'userId, assignmentId, filename are required' });

    // bucket must exist in Supabase: 'submissions' (private)
