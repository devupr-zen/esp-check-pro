...
if (!SUPABASE_URL || !SERVICE_KEY) {
	throw new Error(
		"Missing SUPABASE env vars (NEXT_PUBLIC_SUPABASE_URL/VITE_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY)",
	);
}
...
export default async function handler(req: any, res: any) {
	try {
		...
		if (!userId || !assignmentId || !filename) {
			return res
				.status(400)
				.json({ error: "userId, assignmentId, filename are required" });
		}
